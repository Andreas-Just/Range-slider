...
CreateSlider.prototype._onThumbClickMove = function(event) {
	...
	this._fillElement.style.width =  parseInt(this._thumbElementLeft.style.left) + 5 + 'px';
};

CreateSlider.prototype._onDocumentMousemove = function(event) {
	let maxX = this._xThumbRight;

	this._xThumbRight = this._thumbElementRightPosition.x - this._containerPosition.x - this._thumbElementRight.offsetWidth;
	this._xThumbLeft = event.clientX - this._containerPosition.x - this._thumbElementLeft.clientWidth / 2;
...
function CreateSlider(sliderElement) {
  ...
  `;
	...
	this._fillElement = sliderElement.querySelector('.slider__fill');
	...
	this._fillElement.style.width = this._thumbElementLeftPosition.x - this._containerPosition.x + 5 + 'px';
